...
CREATE FUNCTION validLabDate (
    tid INTEGER,
    labdate DATE
) RETURNS BOOLEAN
BEGIN
    IF (labdate <= (SELECT prescdate
                    FROM tests
                    WHERE tests.testid = tid))
    THEN
        RETURN TRUE;
    ELSE
        RETURN FALSE;
...
WITH appointments AS (
	SELECT date, time, coupleid, pregnum
	FROM appointment
	WHERE pracid IN (SELECT pracid
                    FROM midwife
                    WHERE name = 'Marion Girard')
	AND date <= DATE('2022-03-25')
	AND date >= DATE('2022-03-21')
)
SELECT date, time, hcardid, name, phone
FROM appointments A, mother M
WHERE M.hcardid IN (SELECT C.hcardid
                    FROM couple C
                    WHERE C.coupleid = A.coupleid)
;


WITH testfor AS (
    SELECT coupleid, pregnum
    FROM pregnancy
    WHERE coupleid IN (SELECT coupleid
                        FROM couple
                        WHERE hcardid IN (SELECT hcardid
                                            FROM mother
                                            WHERE name = 'Victoria Gutierrez'))
    AND pregnum = 2
)
SELECT labdate, result
FROM tests
WHERE coupleid = (SELECT coupleid FROM testfor)
  AND pregnum = (SELECT pregnum FROM testfor)
  AND type = 'blood iron'
;

WITH pregduejuly AS (
    SELECT coupleid, pregnum, estdued, M.hemail
    FROM pregnancy P, midwife M
    WHERE P.ppracid = M.pracid
        AND ((EXTRACT(YEAR FROM estdued) = 2022
               AND EXTRACT(MONTH FROM estdued) = 7)
            OR (estdued IS NULL
               AND EXTRACT(YEAR FROM expbirthtf) = 2022
               AND EXTRACT(MONTH FROM expbirthtf) = 7))
)
SELECT H.name, COUNT(coupleid) AS numpregnancies
FROM hcinstitution H LEFT OUTER JOIN pregduejuly P
ON H.hemail = P.hemail
GROUP BY H.name
;

WITH employedmidwife AS (
    SELECT pracid
    FROM midwife M
    WHERE hemail IN (SELECT hemail
                    FROM hcinstitution H
                    WHERE H.name = 'Lac-Saint-Louis')
), coupleundercare AS (
    SELECT coupleid
    FROM pregnancy
    WHERE (ppracid IN (SELECT pracid FROM employedmidwife)
        OR (bpracid IN (SELECT pracid FROM employedmidwife)))
      AND (estdued >= CURRENT DATE
        OR (estdued IS NULL
            AND expbirthtf >= CURRENT DATE))
)
SELECT M.hcardid, M.name, M.phone
FROM mother M, couple C
WHERE M.hcardid = C.hcardid
AND C.coupleid IN (SELECT coupleid FROM coupleundercare)
;

WITH multibabypreg AS (
    SELECT P.coupleid, P.pregnum, COUNT(babyid) AS numbabies
    FROM pregnancy P LEFT OUTER JOIN baby B
    ON (P.coupleid = B.coupleid AND P.pregnum = B.pregnum)
    GROUP BY P.coupleid, P.pregnum
    HAVING COUNT(babyid) > 1
), mothers AS (
    SELECT C.hcardid
    FROM couple C, multibabypreg M
    WHERE C.coupleid = M.coupleid
    GROUP BY C.hcardid
)
SELECT hcardid, name
FROM mother
WHERE hcardid IN (SELECT hcardid FROM mothers)
;

CREATE VIEW midwifeinfo (pracid, midwifename, phone, email, employername, employeraddr) AS
    SELECT M.pracid, M.name, M.phone, M.email, H.name, H.addr
    FROM midwife M, hcinstitution H
    WHERE M.hemail = H.hemail
;

SELECT * FROM midwifeinfo;

SELECT * FROM midwifeinfo
WHERE employername = 'Lac-Saint-Louis';


SELECT appmid, coupleid, time, name, hcardid
...
INSERT INTO notes (noteid, timestamp, content, appmid)
VALUES (?, ?, ?, ?);

SELECT count(noteid)
FROM notes;

SELECT count(testid) FROM tests;

INSERT INTO tests (testid, type, sampledate, pracid, prescdate, coupleid, pregnum)
VALUES (?, ?, ?, ?, ?, ?, ?);

SELECT count(babyid) AS numBabies, EXTRACT(MONTH FROM dateofbirth) AS month
FROM baby
